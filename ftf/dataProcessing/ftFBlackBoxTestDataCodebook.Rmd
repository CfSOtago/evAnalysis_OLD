 ...
 * Anderson, B. (`r 1900 + as.POSIXlt(Sys.Date())$year`) `r params$title`: `r params$subtitle` (File: `r params$fileName`), `r ggParams$pubLoc`.
 ...
 * https://github.com/CfSOtago/GREENGrid/tree/master/dataProcessing/ftf/
...
kableExtra::kable(summary(geoDT), caption = "Summary of test geo data") %>%
  kable_styling()
...
# create derived 
ftfDT <- GREENGrid::createDerivedFtF(ftfDT)
```
...
kableExtra::kable(head(dateFail), caption = "Example rows where date failed to parse") %>% 
  kable_styling()
```
